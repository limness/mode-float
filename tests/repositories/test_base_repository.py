...
class TestBaseRepository:
    ...
    @pytest.mark.asyncio
    async def test_create_one_duplicate_returns_none_and_rolls_back(
        self, repo: TestEntityRepository, db_session: AsyncSession
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_many_inserts_and_returns_pk(
        self, repo: TestEntityRepository, db_session: AsyncSession
    ):
        rows = [
            {'name': 'u1', 'age': 1},
    ...
    @pytest.mark.asyncio
    async def test_get_one_found_and_none(
        self, repo: TestEntityRepository, db_session: AsyncSession
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_all_with_and_without_filters(
        self, repo: TestEntityRepository, db_session: AsyncSession
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_one_updates_values(
        self, repo: TestEntityRepository, db_session: AsyncSession
    ):
