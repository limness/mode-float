...
async def save_regions_to_db(region_polygons: RegionPolygons, db_session: AsyncSession):
    for name, polygons in region_polygons.items():
        try:
            ...
            geopolygon_str = str([[[point[1], point[0]] for polygon in polygons for point in polygon]])
            ...
            if row:
                await db_session.execute(
                    RegionModel.__table__.update().where(RegionModel.name == name).values(
                        area=area, geopolygon=geopolygon, geopolygon_str=geopolygon_str
                    )
                )
            else:
                await db_session.execute(
