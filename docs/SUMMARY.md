...
[Сервис](https://fly-potato.ru) предназначен для агрегирования, визуализации и аналитической обработки данных о полётах гражданских БПЛА по
...
> [!IMPORTANT]
> Для доступа к [сайту](https://fly-potato.ru) в качестве оператора вы можете использовать следующие данные:
> - Логин: `operator_demo`
> - Пароль: `operator_demo`

> [!IMPORTANT]
> Для доступа к [сайту](https://fly-potato.ru) в качестве администратора вы можете использовать следующие данные:
> - Логин: `admin_demo`
> - Пароль: `admin_demo`

> [!IMPORTANT]
> Сайт: https://fly-potato.ru
...
const DATALENS_IDS: Record<string, string | undefined> = {
  overview: normalizeEmbedId(import.meta.env.VITE_DATALENS_OVERVIEW_ID) ?? АЙДИ_ДАШБОРДА_ОБЗОР,
  regions: normalizeEmbedId(import.meta.env.VITE_DATALENS_REGIONS_ID) ?? 'АЙДИ_ДАШБОРДА_РЕГИОНЫ',
  time: normalizeEmbedId(import.meta.env.VITE_DATALENS_TIME_ID) ?? 'АЙДИ_ДАШБОРДА_ВРЕМЯ',
}
```
